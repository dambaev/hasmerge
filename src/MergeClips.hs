...
mergeClipsMain env args p = mergeClipsEntry env args p >>= toRawValue


mergeClipsEntry :: Ptr C'AVS_ScriptEnvironment
       -> Ptr C'AVS_Value
       -> Ptr ()
       -> IO AVS_Value
mergeClipsEntry env rargs params = do
    ...
    liftM toAVSValue $ mergeClips env leftClip rightClip filename
    
    
mergeClips:: Ptr C'AVS_ScriptEnvironment
        -> AVS_Clip
        -> AVS_Clip
        -> String
        -> IO AVS_Clip
mergeClips env left right scriptfile = do
